...
class Application (fix.Application):
    ...
    def run(self):
	print '''
	input 1 to fill the snapshot,public OrderID genOrderID() {
        return new OrderID(Integer.toString(++m_orderID));
    }

	input 2 to seny query order,
	input 3 to quit
	'''
	while True:
		input = raw_input()
    ...
    def fillSnapShot(self):
		home_path = os.getenv('USERPROFILE')+"\snapshotfrompython.txt" if os.getenv('USERPROFILE') != None else  'snapshotfrompython.txt'
		f = file(home_path, "w")
