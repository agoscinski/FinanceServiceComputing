...
CREATE TABLE IF NOT EXISTS `FSCDatabase`.`Order` (
  ...
  PRIMARY KEY (`ClientOrderID`, `Account_CompanyID`, `ReceivedTime`),
...
CREATE TABLE IF NOT EXISTS `FSCDatabase`.`OrderExecution` (
  ...
  `OrderExecutionQuantity` INT NULL,
  `OrderExecutionPrice` DECIMAL(2) NULL,
  `ExecutionTime` TIMESTAMP NULL,
  `MsgSeqNum` INT NULL,
  `Order_BuyClientOrderID` VARCHAR(45) NOT NULL,
  `Order_BuyCompanyID` VARCHAR(20) NOT NULL,
  `Order_BuyReceivedTime` TIMESTAMP NOT NULL,
  `Order_SellClientOrderID` VARCHAR(45) NOT NULL,
  `Order_SellCompanyID` VARCHAR(20) NOT NULL,
  `Order_SellReceivedTime` TIMESTAMP NOT NULL,
  PRIMARY KEY (`ExecutionID`),
  INDEX `fk_OrderExecution_Order1_idx` (`Order_BuyClientOrderID` ASC, `Order_BuyCompanyID` ASC, `Order_BuyReceivedTime` ASC),
  INDEX `fk_OrderExecution_Order2_idx` (`Order_SellClientOrderID` ASC, `Order_SellCompanyID` ASC, `Order_SellReceivedTime` ASC),
  CONSTRAINT `fk_OrderExecution_Order1`
    FOREIGN KEY (`Order_BuyClientOrderID` , `Order_BuyCompanyID` , `Order_BuyReceivedTime`)
    REFERENCES `FSCDatabase`.`Order` (`ClientOrderID` , `Account_CompanyID` , `ReceivedTime`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_OrderExecution_Order2`
    FOREIGN KEY (`Order_SellClientOrderID` , `Order_SellCompanyID` , `Order_SellReceivedTime`)
    REFERENCES `FSCDatabase`.`Order` (`ClientOrderID` , `Account_CompanyID` , `ReceivedTime`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `FSCDatabase`.`OrderCancel` (
  ...
  `Order_ReceivedTime` TIMESTAMP NOT NULL,
  ...
  CONSTRAINT `fk_OrderCancel_Order1`
    FOREIGN KEY (`Order_ClientOrderID` , `Order_Account_CompanyID` , `Order_ReceivedTime`)
    REFERENCES `FSCDatabase`.`Order` (`ClientOrderID` , `Account_CompanyID` , `ReceivedTime`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
